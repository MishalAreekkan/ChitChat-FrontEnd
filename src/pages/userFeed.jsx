...
function UserFeed() {
    ...
    return (
        <div className="p-6 rounded-lg w-full h-full mt-4">
            {posts ? (
                <>
                    {posts.user_posts.length > 0 && (
                        <div>
                            <h3 className="font-bold text-lg mb-4">Your Posts</h3>
                            {posts.user_posts.map((post) => (
                                        ...
                                        <div className="relative">
                                            ...
                                            {isDropdownOpen && (
                                                <div className="absolute top-8 left-0 bg-white border border-gray-200 rounded shadow-lg z-10">
                                                    ...
                                                    <MenuItem onClick={() => handleOptionSelect(2)}>Not interested {post.username || 'Unknown User'}'s Post</MenuItem>
                                                    <MenuItem onClick={() => handleOptionSelect(3)}>Unfollow {post.username || 'Unknown User'} </MenuItem>
                                                    <MenuItem className="text-red px-4 py-2 rounded-lg" onClick={() => handleOptionSelect(3)}>Close</MenuItem>
                                                </div>
                                            )}
                                        </div>
                                    </div>
